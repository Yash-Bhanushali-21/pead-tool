...
|------------|------|---------|
| 2026-05-01 | News intelligence (Issue #2) | **Hedge-fund-grade news pipeline** on branch `issue-2-fix`. New: `source_registry.py` (Tier 1–4 credibility weights for 50+ India sources), `dedup.py` (title-fingerprint syndication collapse), `event_classifier.py` (150+ keyword rules → EARNINGS/GUIDANCE/ANALYST_RATING/CORPORATE_ACTION/M_AND_A/REGULATORY/MANAGEMENT/MACRO_INDIA/SECTOR_CONTEXT/GENERAL), `news_signals.py` (news_velocity, source_agreement, event_tone), `exchange_announcements.py` (NSE corporate filings fetcher). Enhanced: `collector.py` adds 7 built-in Tier-2 India RSS feeds (NDTV Profit, Mint, Business Standard, BusinessLine, ET Markets, MoneyControl Results, Financial Express) + title-dedup at collect(); `sentiment_pipeline.py` rewrites scoring to source-credibility × event-type weighted mean + coverage_meta + news_signals in every response; `layer.py` + `market_sentiment_layer.py` thread window args. New pipeline stage `run_exchange_announcements` in `EQUITY_RESEARCH_FULL_STAGES`. New standalone route `POST /api/tools/run/exchange-announcements`. Config: `NEWS_DEDUP_TITLE_ENABLED`, `NEWS_EXCHANGE_ANNOUNCEMENTS_ENABLED`, `NEWS_EXCHANGE_ANNOUNCEMENTS_TIMEOUT_S`, `NEWS_SOURCE_WEIGHTS`. Frontend: created all 4 missing `web/src/lib/` files; stage picker gets `run_exchange_announcements` label + "News intelligence" preset; tool page gets exchange announcements checkbox. |
